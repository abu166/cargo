...
export function Transit({ theme = 'light' }: TransitProps) {
  ...
  const fetchShipments = async () => {
    ...
    try {
      // Fetch incoming
      const resIncoming = await authFetch(`/api/shipments?type=incoming&station=${user.station}`);
      const dataIncoming = await resIncoming.json();
      setIncomingShipments(Array.isArray(dataIncoming) ? dataIncoming : []);

      // Fetch outgoing
      const resOutgoing = await authFetch(`/api/shipments?type=outgoing&station=${user.station}`);
  ...
  const handleScan = async () => {
    ...
    setProcessing(true);
    try {
      const response = await authFetch(`/api/shipments/${scanInput}/transit`, {
        method: 'POST',
