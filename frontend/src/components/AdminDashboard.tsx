...
export function AdminDashboard({ theme = 'light' }: AdminDashboardProps) {
  ...
  const handleSyncUsers = async () => {
    try {
      const response = await authFetch('/api/admin/sync-users', { method: 'POST' });
      if (!response.ok) {
  ...
  const handleDeleteEmployee = async (id: string) => {
    if (confirm(t('deleteEmployeeConfirm'))) {
      try {
        const response = await authFetch(`/api/admin/employees/${id}`, {
          method: 'DELETE'
        });
        ...
        <div className="flex flex-col md:flex-row gap-4 justify-between items-start md:items-center">
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              className={`px-4 py-2 rounded-lg border whitespace-nowrap ${isDark ? 'border-gray-700 text-gray-300 hover:bg-gray-800' : 'border-gray-200 text-gray-700 hover:bg-gray-50'}`}
            >
              Sync Users
            </button>
            <button
