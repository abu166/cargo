...
func (s *Server) handleMarkTransit(w http.ResponseWriter, r *http.Request) {
	...
	shipment, err := s.services.Shipments.MarkTransit(r.Context(), chi.URLParam(r, "id"), req.CurrentStation, req.OperatorID, req.OperatorName)
	if err != nil {
		handleServiceError(w, err)
		return
	}
...
func (s *Server) handleLegacyTransit(w http.ResponseWriter, r *http.Request) {
	...
	if req.CurrentStation == current.FromStation && (current.ShipmentStatus == model.ShipmentReadyForLoading || current.ShipmentStatus == model.ShipmentPaid) {
		if current.ShipmentStatus == model.ShipmentPaid {
			if _, err := s.services.Shipments.ReadyForLoading(r.Context(), current.ID, req.OperatorID, req.OperatorName); err != nil {
				handleServiceError(w, err)
				return
			}
		}
		shipment, err := s.services.Shipments.Load(r.Context(), current.ID, req.OperatorID, req.OperatorName, &req.CurrentStation, nil)
		if err != nil {
			handleServiceError(w, err)
			return
		}
	...
	if req.CurrentStation == current.FromStation && current.ShipmentStatus == model.ShipmentLoaded {
		shipment, err := s.services.Shipments.Dispatch(r.Context(), current.ID, req.OperatorID, req.OperatorName, &req.CurrentStation)
		if err != nil {
			handleServiceError(w, err)
			return
		}
