...
func TestPilotLifecycleFlow(t *testing.T) {
	...
	calcResp := performJSON(t, server.Router(), "POST", "/api/shipments/"+shipment.ID+"/calculate-tariff", map[string]any{})
	if calcResp.Code != http.StatusOK {
		t.Fatalf("calculate tariff failed: %d %s", calcResp.Code, calcResp.Body.String())
	}

	sendToPaymentResp := performJSON(t, server.Router(), "POST", "/api/shipments/"+shipment.ID+"/send-to-payment", map[string]any{})
	...
	confirmResp := performJSON(t, server.Router(), "POST", "/api/payments/"+payment.ID+"/confirm", map[string]any{})
	if confirmResp.Code != http.StatusOK {
		t.Fatalf("confirm payment failed: %d %s", confirmResp.Code, confirmResp.Body.String())
	}

	qrResp := performJSON(t, server.Router(), "POST", "/api/shipments/"+shipment.ID+"/generate-qr", map[string]any{})
	if qrResp.Code != http.StatusOK {
		t.Fatalf("generate qr failed: %d %s", qrResp.Code, qrResp.Body.String())
	}

	readyResp := performJSON(t, server.Router(), "POST", "/api/shipments/"+shipment.ID+"/ready-for-loading", map[string]any{})
	...
	readyIssueResp := performJSON(t, server.Router(), "POST", "/api/shipments/"+shipment.ID+"/ready-for-issue", map[string]any{})
	...
	closeResp := performJSON(t, server.Router(), "POST", "/api/shipments/"+shipment.ID+"/close", map[string]any{})
